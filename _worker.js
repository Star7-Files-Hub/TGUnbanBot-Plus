...
function scoreAdProfile(profile, options = {}) {
	...
	// ===== 私有群一次性邀请链接（2026-09-09 方案 C；2026-09-24 拆成「孤立 / 有旁证」两档）=====
	// 加分判据与豁免剔除【刻意挨着写】：剔除的前提就是「这次确实命中了私有形态」，
	// 拆到两处早晚只改一半 —— 那时会出现「加了 5 分但豁免还照减 3 分」的半吊子状态。
	// 两档分值的完整推演、以及为什么它不会滚成累积误封，见 AD_PRIVATE_INVITE_SCORE。
	//
	...
	if (privateInvite) {
		...
		// 三种都没有，就只到观察线 —— 不封、不禁言、不推快照、不进历史分，对正常用户无感。
		// 旁证取「本次评分里已经算过的信号」，不额外发明判据：
		//   ① 招揽 / 利益文案（hasAdSolicitCopy）
		//   ② 机器生成型昵称（与下面 AD_GENERATED_NAME_SCORE 用的是同一对判据）
		//   ③ 交易动词 × 业务关键词同现（与上面 AD_KEYWORD_COMBO_SCORE 用的是同一组命中）
		const solicitCopy = hasAdSolicitCopy(combined);
		const generatedName = Boolean(displayName)
			&& (isAdGeneratedWesternName(displayName) || isAdRandomExoticName(displayName));
		const tradeCombo = tradeHits.length > 0 && businessHits.length > 0;
		const corroborated = solicitCopy || generatedName || tradeCombo;
		if (corroborated) {
			add(AD_PRIVATE_INVITE_CORROBORATED_SCORE, '资料卡含私有群一次性邀请链接（t.me/+ 或 joinchat）+ 广告旁证：'
...
// 不含资料分】。所以这几分不进历史分，不会出现「第一条 observe、第二条再叠一份成倍数分」
// 的恶性循环 —— 那正是 @MiLov1900 两次被误封的机制，已于 2026-09-08 根治，这里不能把它请回来。
//
// ===== 【2026-09-24 第三轮：7 拆成「孤立链接 5 / 有旁证 7」】=====
// 主人原话：「频道私聊的判定再优化下，降低误封概率」。
//
// 触发这一轮的线上数据：部署后 14:57 / 15:31 / 16:11 四次判定里，score 只有 4~5 的
// 正常用户被 AI 层硬命中封掉；而 /ignore 放行库里躺着的全是同一形态的正常资料卡 ——
//   `频道私聊：https://t.me/+…`、`私聊请通过这个联系 https://t.me/+…`、
//   `https://t.me/+… 频道私聊`、`私信请通过频道https://t.me/+…`
// 第二轮注释里「正常用户是不会放私有群在用户简介上的」这个前提，被主人自己的 /ignore
// 记录推翻了。离线把这 8 条真实正常资料卡喂进 scoreAdProfile，**其中 4 条直接得 7 分撞线**
// （`私信请通过频道…`、纯链接、`读书会交流群 …`、`有事请联系频道：…`）——
// 也就是说「+7 单独撞线」这条规则本身就在批量误封，跟 AI 层无关。
//
// 【为什么不能简单改成 5】离线对照 15 条真广告资料卡（指纹库现存样本）后发现：
// `小林`「了解详情点击这里 …收钱…吃肉」与 `🌟此号不回复`「加入2大裙 …」的**唯一**
// 得分来源就是这 +7 —— 它们的话术在 AD_TRADE_VERBS × AD_BUSINESS_KEYWORDS 里逐词零命中。
// 所以 7 分是在替「招揽型文案识别不足」兜底。单纯降到 5 会把这批真广告放进观察窗口。
//
// 【落地方案：链接算「引流信号」，文案才算「广告意图」】
//   孤立链接（资料卡里除链接外只有联系方式/身份说明）→ +5，只到观察线：不封、不禁言、
//     不推快照、不进历史分（retainScore 只含行为分），对正常用户完全无感。
//   链接 + 任一广告旁证 → +7，照旧撞封禁线。旁证三种（见 scoreAdProfile 里的 corroborated）：
//     ① 招揽 / 利益文案（AD_SOLICIT_COPY_VERBS × AD_SOLICIT_COPY_TARGETS）
//     ② 机器生成型昵称（西方全名 / 非常用书写系统随机串）
//     ③ 交易动词 × 业务关键词同现
//
// 【离线回归】8 条真实正常资料卡：4 条落到 0~2 分（豁免词生效），4 条落到 5 分观察线，
// **零条撞 7 分**。15 条真广告资料卡：**全部经旁证升到 7 分及以上**，一条不漏。
const AD_PRIVATE_INVITE_SCORE = 5;
// 私有邀请链接 + 广告旁证时的分值。维持第二轮主人定的 7（= 封禁线）。
const AD_PRIVATE_INVITE_CORROBORATED_SCORE = 7;

// ===== 招揽型文案词表（2026-09-24 新增）=====
// 只用来判断「这条资料卡/正文是不是在招揽人做某事」，即上面那个 corroborated 的旁证①。
// 【刻意不参与计分】它不是独立的广告证据 —— 正常用户也会写「进群了解」。它只负责回答
// 「这条链接是广告的载荷，还是用户自己的联系方式」。所以误命中的代价被限制在
// 「资料卡里同时有私有群链接的人被升回 7 分」这个很窄的面内。
//
// 词表来自线上真广告资料卡（指纹库 15 条）与正常资料卡的对照，取的是**行动指令**：
//   `加群看项目 一天赚8千` / `点击下方链接进群了解更多详细赚钱流程` / `进群了解详情`
//   `一天稳定5万来找我小白也可以带` / `包..养..大..学..生…进：… 代发：@fjdf18`
//   `免.税水.果18pro 只要6k多` / `全.新免.税苹.果18找我.定` / `接预.定`
// 正常资料卡里一个都不出现：
//   `频道私聊` / `私聊请通过这个联系` / `私信请通过频道` / `有事请联系频道` / `读书会交流群`
// 注意「联系」刻意【不在】动词表里 —— 它是正常用户说明联系方式时最常用的词。
// 招揽 / 导流指令词。收录标准是「正常用户在个人简介里不会这么写」，
// 不是「这个词看起来像不像广告」—— 因为它的唯一作用是给私有群链接判旁证。
//   '此号不回复'：广告号的固定免责声明，语义等价于「别私聊，走链接」，本身就是导流指令。
//     线上实例「亚博 此号不回复！！！24h做单入口: https://t.me/+… 💎」。
//   '联系' / '私聊' / '私信' 刻意【不在】表里 —— 它们是正常用户说明联系方式最常用的词，
//     正是本次要救回来的那一类资料卡（`频道私聊：…`、`私聊请通过这个联系 …`）。
const AD_SOLICIT_COPY_VERBS = [
	'加群', '加入', '进群', '入群', '点击', '了解', '详情', '查看', '看看',
	'找我', '来找', '私我', '上车', '代发', '戴发', '预定', '预订', '接预',
	'领取', '免费领', '只要', '限时', '加我', '此号不回复'
];
// 利益 / 标的承诺。同样刻意排除宽泛词：「群」「钱」「万」单独出现太常见
// （「技术交流群」「不值几个钱」），会把技术群主一起卷进来。
// '大裙' 是例外 —— 「裙」是广告圈对「群」的固定谐音写法，正常用户不会写「加入2大裙」，
// 线上真广告资料卡「🌟此号不回复 加入2大裙 https://t.me/+…」正是靠它凑出旁证。
// '做单' 同理：刷单 / 跑分行业的成交黑话，正常简介不用。
const AD_SOLICIT_COPY_TARGETS = [
	'项目', '赚', '日入', '月入', '过万', '收入', '收钱', '吃肉', '福利', '资源',
	'小白', '包养', '学生', '苹果', '水果', '免税', '价格', '流程', '一单', '兼职',
	'大裙', '做单'
];

// 招揽文案判定。两个条件任一成立即算：
//   ① 招揽动词 × 利益词同现（与 AD_KEYWORD_COMBO_SCORE 的组合式设计一致）
//   ② 招揽动词出现 ≥3 个【不同】词 —— 纯 CTA 型资料卡没有利益词，但正常用户不会在
//      简介里连写三个行动指令。线上实例：「了解详情点击这里」「进群了解详情」。
//      门槛卡 3 而不是 2：`想了解可以进群` 这种正常写法只有 2 个，不能升档。
function hasAdSolicitCopy(text) {
	const raw = String(text ?? '');
	if (!raw) return false;
	// 三个变体一起看：原文、现有去混淆（buildAdMatchTexts，仅在命中签名时给第二份）、
	// 以及【剥掉全部分隔符】的一份。
	//
	// 为什么非要第三份：AD_CHAR_SPLIT_RE_* 那两条签名要求「分隔符恰好 1 个」且「分组长度稳定」，
	// 而线上广告资料卡最常见的三种写法【一条都不命中】：
	//   `包..养..大..学..生`（双点）、`全…掴…包…养`（省略号 U+2026 根本不在分隔符类里）、
	//   `免。税苹。果１8接预.定`（1 字组与 2 字组混排打断 {1,2} 重复）。
	// 去改那两条正则会牵动全文件的词表匹配（buildAdMatchTexts 到处在用），
	// 所以这里只给本判据做局部归一化 —— 命中面窄，且不改动任何既有检测行为。
	const variants = new Set(buildAdMatchTexts(raw));
	variants.add(raw.replace(/[.\u00b7\u30fb\uff65\uff61\u2027\u2219\u22c5\u2022_\-\u2014\u2013~\uff5e*\s\u3002\uff64\u3001,\uff0c\u2026]+/gu, ''));
	for (const t of variants) {
		if (!t) continue;
		const verbs = countAdKeywordHits(t, AD_SOLICIT_COPY_VERBS);
		if (!verbs.length) continue;
		if (countAdKeywordHits(t, AD_SOLICIT_COPY_TARGETS).length > 0) return true;
		if (new Set(verbs).size >= 3) return true;
	}
	return false;
}
